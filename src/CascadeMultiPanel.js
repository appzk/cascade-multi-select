...
class CascadeMultiPanel extends React.Component {
  static getDerivedStateFromProps(props, state) {
    ...
    const { dataList, allItemDisabled } =
      CascadeMultiPanel.setData(value, options, keyCouldDuplicated);
  ...
  static setData(value, options, keyCouldDuplicated) {
    ...
    if (dataList && dataList.length) {
      const res = CascadeMultiPanel.setCleanResult(dataList, true, keyCouldDuplicated);
      dataList = res.listArray;
      allItemDisabled = res.allItemDisabled;
      for (let i = 0, len = value.length; i < len; i += 1) {
        const $id = value[i];
        const treeNodeObj =
          CascadeMultiPanel.getTreeNodeData(dataList, $id, null, keyCouldDuplicated);
        if (treeNodeObj) {
          const { parentNode, itemNode } = treeNodeObj;
          itemNode.checked = true;
          if (itemNode.children) {
            itemNode.children = CascadeMultiPanel.setChildrenChecked(
              itemNode.children,
              true,
              true,
            );
          ...
          if (parentNode) {
            CascadeMultiPanel.setFatherCheckState(itemNode, true, dataList, keyCouldDuplicated);
          }
        }
      }
    }
    ...
    if (dataList && dataList.length) {
      for (let i = 0, len = dataList.length; i < len; i += 1) {
        ...
        if (dataList[i].children) {
          const item =
            CascadeMultiPanel.getTreeNodeData(
  ...
  static setFatherCheckState(item, checked, dataList, keyCouldDuplicated) {
    ...
    if (parentNode) {
      ...
      CascadeMultiPanel.setFatherCheckState(parentNode, checked, dataList, keyCouldDuplicated);
    }
  }
  ...
  onItemChecked(item, level) {
    ...
    if (!isCleanDisabledLabel && this.itemDisabledNodes.length > 0) {
      ...
      while (itemDisabledNode) {
        CascadeMultiPanel.setFatherCheckState(
  ...
  onCleanSelect() {
    ...
    this.setState({
      dataList: CascadeMultiPanel.setCleanResult(
  ...
  onDeleteItem(item, level) {
    ...
    if (level) {
      CascadeMultiPanel.setFatherCheckState(itemNode, false, dataList, keyCouldDuplicated);
    }
    this.setSelectResult();
  }
  ...
  renderUlList(level) {
    ...
    const treeNodeObj = CascadeMultiPanel.getTreeNodeData(
      dataList,
      selectArray[level - 1],
      null,
      keyCouldDuplicated
...
CascadeMultiPanel.displayName = 'CascadeMultiPanel';
polyfill(CascadeMultiPanel);

export default CascadeMultiPanel;
// module.exports = CascadeMultiPanel;

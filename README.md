...
```sh
git clone https://github.com/ondc/cascade-multi-select
cd cascade-multi-select
npm install
npm run start
```
...
```javascript
import CascadeSelect from 'CascadeMultiPanel'
...
```javascript
import CascadeSelect from 'CascadeMultiSelect'
import { render } from 'react-dom'
...
|---|---|---|---|---|
| prefixCls | string | false | `'yg-cascader'` | 默认的类名前缀 |
...
```less
.yg-cascader-submenu-empty,
.yg-dropdown-menu-submenu {
